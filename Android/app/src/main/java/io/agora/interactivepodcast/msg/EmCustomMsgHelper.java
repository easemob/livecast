package io.agora.interactivepodcast.msg;
 ...
 * （3）设置自定义消息监听{@link #setOnCustomMsgReceiveListener(com.easemob.custommessage.OnCustomMsgReceiveListener)}
 *      用于接收不同的自定义消息类型（目前仅礼物，点赞及弹幕消息）。
 * （4）发送自定义消息：
 *      a、如果自定义消息类型与library相同，且所传参数相同或者相近，可以直接调用如下方法：
 *      {@link #sendGiftMsg(String, int, com.easemob.custommessage.OnMsgCallBack)},
 *      {@link #sendPraiseMsg(int, com.easemob.custommessage.OnMsgCallBack)},
 *      {@link #sendBarrageMsg(String, com.easemob.custommessage.OnMsgCallBack)} 或者
 *      {@link #sendGiftMsg(Map, com.easemob.custommessage.OnMsgCallBack)},
 *      {@link #sendPraiseMsg(Map, com.easemob.custommessage.OnMsgCallBack)},
 *      {@link #sendBarrageMsg(Map, com.easemob.custommessage.OnMsgCallBack)}
 *      b、如果有其他自定义消息类型，可以调用如下方法：
 *      {@link #sendCustomMsg(String, Map, com.easemob.custommessage.OnMsgCallBack)},
 *      {@link #sendCustomMsg(String, EMMessage.ChatType, String, Map, com.easemob.custommessage.OnMsgCallBack)}。
