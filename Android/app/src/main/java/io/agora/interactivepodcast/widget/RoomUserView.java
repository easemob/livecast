...
//        if ("add+".equals(user.getName()))
        if (user.getName().contains("号麦")) {
            //TODO 显示加号
            if (this.mMember == null || this.mMember.getUserId() == null || !TextUtils.equals(this.mMember.getUserId().getAvatar(), user.getAvatar())) {

                Glide.with(this)
                        .load(R.mipmap.add_member)
                        .placeholder(R.mipmap.default_head)
                        .error(R.mipmap.default_head)
                        .circleCrop()
                        .into(mDataBinding.ivUser);
                mDataBinding.ivMaster.setVisibility(GONE);
            ...
            if (ObjectsCompat.equals(user, member.getRoomId().getAnchorId())) {
                mDataBinding.ivMaster.setVisibility(VISIBLE);
