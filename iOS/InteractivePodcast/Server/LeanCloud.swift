...
extension Room {
    ...
    static func from(object: LCObject) throws -> Room {
        ...
        let chatGroupId = object.get(CHAT_GROUP_ID)!.stringValue!
        let room = Room(id: objectId, channelName: channelName, anchor: anchor, chatGroupId: chatGroupId)
    ...
    static func create(room: Room) -> Observable<Result<String>> {
        return Database.save {
            ...
            try object.set(CHAT_GROUP_ID, value: room.chatGroupId)
            return object
        }
    }
    ...
    static func getRooms() -> Observable<Result<Array<Room>>> {
        return Database.query(className: TABLE) { (query) in
            ...
            try query.where(CHAT_GROUP_ID, .selected)
    ...
    static func getRoom(by objectId: String) -> Observable<Result<Room>> {
        return Database.query(className: TABLE, objectId: objectId) { query in
            ...
            let chatGroupId: String = data.get(CHAT_GROUP_ID)!.stringValue!
            return Room(id: objectId, channelName: channelName, anchor: anchor, chatGroupId: chatGroupId)
        }
    }
