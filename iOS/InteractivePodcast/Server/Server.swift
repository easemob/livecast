...
extension Server: Service {
    ...
    func getAccount() -> Observable<Result<User>> {
        if (account == nil) {
            ...
            if (user != nil) {
                return User.getUser(by: user!.id).flatMap { result in
                    return result.onSuccess {
                        self.account = result.data!
                        return ChatServer.shared().registerAndLogin(user!.id).map { result -> Result<User> in
                            if result.success {
                                return Result<User>(success: true, data: self.account!)
                            }
                            return Result<User>(success: false, message: result.message)
                        }
                    }
                }
            } else {
                return User.randomUser().flatMap { result in
                    return result.onSuccess {
                        self.account = result.data!
                        return ChatServer.shared().registerAndLogin(self.account!.id).flatMap { result in
                            return result.onSuccess {
                                return CoreData.saveAccount(user: self.account!)
                            }
                        }
                    }
                }
            ...
            return ChatServer.shared().login(account!.id).flatMap { result -> Observable<Result<User>> in
                if result.success {
                    return Observable.just(Result(success: true, data: self.account!))
                } else {
    ...
    func create(room: Room) -> Observable<Result<Room>> {
        if let user = account {
            room.anchor = user
            return ChatServer.shared().createGroupChat(chatName: room.id).concatMap({ result in
                return result.onSuccess {
                    room.chatGroupId = result.data!
...
//                    .concatMap { result -> Observable<Result<Room>> in
//                        return ChatServer.shared().joinGroup(groupId: room.chatGroupId).concatMap { result ->  Observable<Result<Room>>  in
//                            return result.onSuccess {
//                                return Observable.just(Result(success: true, data: room))
//                            }
//                        }
