        ...
        return Server.shared().getRooms()
            ...
            .map { [unowned self] data in
                ...
                if (data.success) {
                    ...
                    let rms = (data.data ?? []).filter({$0.total > 0})
                    self.roomList.append(contentsOf: rms)
                }
                return data
...
//    }
    ...
    func createRoom(with name: String) -> Observable<Result<Room>> {
        ...
        if let anchor = account {
            return Server.shared().create(room: Room(id: "", channelName: name, anchor: anchor, chatGroupId: ""))
        } else {
    ...
    func join(room: Room) -> Observable<Result<Room>> {
        ChatServer.shared().joinGroup(groupId: room.chatGroupId).subscribe(onNext: { result in
            print("加入群聊成功",result.success)
        }).disposed(by: bag)
